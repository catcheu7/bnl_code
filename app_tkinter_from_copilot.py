...
class MainWindow:
    def __init__(self, root):
        self.root = root
        self.root.title("GDS Voxels v.0.01")

        # Widgets
        self.load_button = Button(root, text="Load Design", command=self.load_design)
        self.load_button.pack()

        self.file_label = Label(root, text="No file selected")
        self.file_label.pack()

        self.x1_label = Label(root, text="X: Starting")
        self.x1_label.pack()
        self.x1_entry = Entry(root)
        self.x1_entry.pack()

        self.x2_label = Label(root, text="X: Ending")
        self.x2_label.pack()
        self.x2_entry = Entry(root)
        self.x2_entry.pack()

        self.y1_label = Label(root, text="Y: Starting")
        self.y1_label.pack()
        self.y1_entry = Entry(root)
        self.y1_entry.pack()

        self.y2_label = Label(root, text="Y: Ending")
        self.y2_label.pack()
        self.y2_entry = Entry(root)
        self.y2_entry.pack()

        self.confirm_button = Button(root, text="Confirm", command=self.graphset)
